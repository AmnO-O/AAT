...
MODEL_PATH    = "model_v7.pth"
BEST_PPO_PATH = "model_v7_best.pth"
...
class BomberNet(nn.Module):
    ...
    def forward(self, x):
        ...
        v = torch.relu(self.value_conv(feat.detach()))  # detach: backbone trains only on policy signal
...
def build_train_opponents(controlled_id, opp_seed, frozen_model, league_pool, round_idx):
    ...
    if round_idx < 10:
        p_frozen=0.55; p_league=0.25; p_weak=0.20; p_medium=0.00; p_strong=0.00
    elif round_idx < 30:
