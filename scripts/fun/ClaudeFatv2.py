"""
ClaudFat_v5.py — Bomberland training pipeline
...
PPO_EPOCHS              = 6
PPO_BATCH_SIZE          = 256
PPO_CLIP_EPS            = 0.15  # FIX: reduced from 0.20 — prevents oscillation
PPO_GAMMA               = 0.98
PPO_LAMBDA              = 0.95
PPO_VALUE_COEF          = 0.5
PPO_ENTROPY_COEF        = 0.03  # FIX: raised from 0.01 — annealed per round below
PPO_ENTROPY_DECAY       = 0.80  # multiply per round; floor at 0.005
PPO_ENTROPY_MIN         = 0.005
PPO_MAX_GRAD_NORM       = 1.0
BC_MIX_COEF             = 0.05
LEAGUE_POOL_SIZE        = 8     # FIX: raised from 4

# --- Multiprocessing rollout
NUM_ROLLOUT_WORKERS     = max(1, min(4, (os.cpu_count() or 1) - 1))
# Use spawn to avoid CUDA fork issues; falls back to single-process on failure.
...
# ===========================================================================
# Model (unchanged from v4 — spatial/scalar split + 4×4 pool)
# ===========================================================================
...
class BomberNet(nn.Module):
    """
    Actor-critic with spatial CNN path (20 channels) + scalar MLP path (7 channels).
...
def build_selfplay_opponents(
    ...
    FIX v5: 50% frozen-current / 30% league / 20% baselines.
    Baselines weighted toward tactical & genius (top scorers on leaderboard).
    """
    rng = random.Random(game_seed)
    # Weighted baseline pool: tactical appears 3x, genius 2x, smarter 1x
    base_pool = []
    for cls, weight in [(TacticalRuleAgent, 3), (GeniusRuleAgent, 2), (SmarterRuleAgent, 1)]:
        if cls is not None:
    ...
    for pid in [p for p in range(4) if p != controlled_id]:
        r = rng.random()
        if r < 0.50 and frozen_model is not None:
...
# ===========================================================================
# PPO fine-tuning  — FIX v5: entropy_coef passed per-round for annealing
# ===========================================================================
def ppo_finetune(
    model: nn.Module,
    episodes: List[RolloutEpisode],
    ...
    for epoch in range(1, PPO_EPOCHS + 1):
        ...
        for start in range(0, N, PPO_BATCH_SIZE):
            ...
            # FIX: use annealed entropy_coef passed as argument
...
# ===========================================================================
# Evaluation  — FIX v5: randomized seeds
# ===========================================================================
def quick_eval_against_baselines(
    ...
    if seed_offset is None:
        seed_offset = random.randint(0, 500000)  # FIX: randomize eval seeds
...
def main() -> None:
    ...
    league.set_anchor(model)   # FIX: pin BC model as permanent anchor
    league.add(model)

    # Compute initial entropy coefficient (annealed per round)
    ent_coef = float(PPO_ENTROPY_COEF)

    for round_idx in range(RL_ROUNDS):
        print(f"--- PPO round {round_idx + 1}/{RL_ROUNDS} | ent_coef={ent_coef:.4f} ---",
              flush=True)
        ...
        model = ppo_finetune(model, rollouts, bc_mix_dir=TRAIN_DIR, entropy_coef=ent_coef)
        league.add(model)
